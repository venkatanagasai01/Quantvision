...
export function RecommendationTimeline({ trades }: { trades: any[] }) {
  ...
  return (
    <div className="bg-[#0B1120] border border-white/5 rounded-xl shadow-sm p-6 h-[300px] flex flex-col">
      <h3 className="text-sm font-bold text-white uppercase tracking-wider mb-6">Recommendation History</h3>
          ...
          {sortedTrades.map((trade, i) => {
            ...
            return (
              <div key={i} className="relative pl-6">
                <div className={`absolute w-3 h-3 rounded-full -left-[7px] top-1.5 border-2 border-[#0B1120] ${isBuy ? 'bg-emerald-500' : 'bg-red-500'}`} />
                <div className="flex justify-between items-start">
                  <div>
                    <span className={`text-xs font-bold uppercase tracking-widest ${isBuy ? 'text-emerald-400' : 'text-red-400'}`}>
                      {trade.action} SIGNAL
                    </span>
                    <p className="text-sm text-slate-300 mt-0.5">Executed at ${trade.price?.toFixed(2)}</p>
                  </div>
