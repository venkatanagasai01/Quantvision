...
function ScoreCard({ score, icon: Icon, title, shortDesc, longDesc, inverse = false }: ScoreCardProps) {
  ...
  return (
    <div className={`relative bg-[#0B1120] border-white/5 border rounded-2xl shadow-sm overflow-hidden hover:shadow-md transition-shadow ${grade ? `${grade.bg} ${grade.glow}` : "border-white/5"}`}>
      {/* Top colour strip */}
      {grade && (
      ...
      <div className="p-5 flex flex-col gap-4">
        ...
        <div className="flex items-center gap-2">
          <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${grade?.bg || "bg-[#030712] border border-white/5"}`}>
            <Icon className={`w-4 h-4 ${grade?.color || "text-slate-400"}`} />
          </div>
          <div>
            <div className="text-xs font-black text-slate-300 uppercase tracking-wider">{title}</div>
        ...
        {inverse && val !== null && (
          <div className="text-[10px] text-slate-400 font-medium border-t border-white/5 pt-2">
