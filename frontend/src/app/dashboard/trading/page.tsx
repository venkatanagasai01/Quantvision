    ...
    <div className="max-w-[1200px] mx-auto flex flex-col gap-8 pb-10 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 border-b border-slate-200 pb-6">
        <div>
          ...
          <p className="text-slate-400 mt-2 text-sm leading-relaxed max-w-xl">
            Simulate real market trades with <strong className="text-white">₹10,00,000 virtual capital</strong>. Practice strategies, track P&L, and build confidence — with zero financial risk.
          </p>
        </div>
        <div className="flex items-center gap-3 shrink-0">
          <div className="flex items-center gap-2 px-3 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-lg text-sm">
            ...
            <span className="text-emerald-400 font-semibold text-xs">Simulated Trading Active</span>
          </div>
        </div>
      </div>
      ...
      {showBanner && (
        <div className="info-banner flex items-start justify-between gap-4">
          <div className="flex items-start gap-3">
            ...
            <div>
              <div className="font-bold text-indigo-300 text-sm mb-1">How Paper Trading Works</div>
              ...
              <div className="flex flex-wrap gap-4 mt-3">
                {[
                  { label: "Starting Capital", value: "₹10,00,000", color: "text-indigo-400" },
                  { label: "Risk",             value: "Zero",        color: "text-emerald-400" },
                  { label: "Prices",           value: "Real Market", color: "text-slate-300" },
                  { label: "Orders",           value: "Instant Fill",color: "text-slate-300" },
                ].map((s, i) => (
                  <div key={i} className="text-center">
                    <div className={`text-base font-black ${s.color}`}>{s.value}</div>
                    <div className="text-xs text-indigo-400/70 font-semibold">{s.label}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 flex flex-col gap-8">
          ...
          {/* Open Positions */}
          <section className="bg-[#0B1120] border border-white/5 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="flex flex-col gap-8">
          ...
          <div className="bg-gradient-to-br from-indigo-500/10 to-transparent border border-indigo-500/20 rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="text-sm font-black text-white uppercase tracking-wider">How to Place a Trade</h3>
            </div>
            <div className="flex flex-col gap-4">
              {[
                ...
                <div key={i} className="flex items-start gap-3">
                  <div className="step-number shrink-0 !border-white/10 !bg-[#0B1120] text-slate-300">{s.n}</div>
                  <div>
